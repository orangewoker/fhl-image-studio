...
echo [FHL Studio] Windows portable UI launcher V2.0.1
echo.
...
if exist "%LOCAL_CONFIG%" (
  ...
  echo [FHL Studio] Local FHL API config is ready.
) else (
  if exist "%FRONTEND_FHL_CONFIG%" del /F /Q "%FRONTEND_FHL_CONFIG%" >nul 2>nul
  echo [FHL Studio] Local FHL API config was not found; use the in-app FHL API button to configure it.
)

if exist "%PORTABLE_NODE%" (
  set "NODE_EXE=%PORTABLE_NODE%"
  set "PORTABLE_MODE=1"
  echo [FHL Studio] Using bundled Node runtime.
) else (
  ...
  :node_found
  echo [FHL Studio] Using system Node.js.
)
...
if not exist "node_modules\vite\bin\vite.js" (
  ...
  echo [FHL Studio] Dependencies are missing.
  echo [FHL Studio] Installing dependencies with npm.cmd install...
...
echo [FHL Studio] Starting V2.0.1 Windows UI preview...
echo [FHL Studio] URL: %URL%
echo [FHL Studio] User files root: %PUBLIC_ROOT%
echo [FHL Studio] Port %PORT% is strict. Close old previews if it is busy.
echo.
