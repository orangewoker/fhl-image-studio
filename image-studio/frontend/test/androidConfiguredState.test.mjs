...
test("Android header brand opens about sheet with fork and original GitHub links", () => {
  ...
  assert.match(header, /title="关于 Ai Image"/);
  assert.match(header, /aria-label="关于 Ai Image"/);
  ...
  assert.match(brandAboutSheet, /ANDROID_FHL_REPO_URL = "https:\/\/github\.com\/orangewoker\/fhl-image-studio"/);
  assert.match(brandAboutSheet, /ANDROID_ORIGINAL_REPO_URL = "https:\/\/github\.com\/RoseKhlifa\/Image-Studio"/);
  assert.match(brandAboutSheet, /const ANDROID_BRAND_VERSION = `V\$\{appVersion\}`;/);
  assert.match(brandAboutSheet, /Ai Image GitHub/);
