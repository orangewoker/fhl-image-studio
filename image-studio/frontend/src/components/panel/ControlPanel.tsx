...
export function ControlPanel() {
  const {
    apiKey, mode, prompt, optimizationGuidance, negativePrompt, size, quality, seed, styleTag,
    outputFormat, batchCount,
    sources, currentImage, reversePromptImage,
    errorMessage, errorRawPath, isRunning, lastPayload, isTestingKey, isOptimizingPrompt, isReversingPrompt,
    apiMode, requestPolicy, baseURL, profiles, imageModelID,
    setField, clearError, pushToast,
    selectSourceImage, importSourceImageFile, selectReversePromptImage, importReversePromptImageFile, clearReversePromptImage, removeSource, clearSources,
    openUpstreamConfig,
    submit, cancel, retryLast, optimizePrompt, reversePromptFromImage,
  } = useStudioStore();
  ...
  const reverseReady = !!(
    (hasUsableResponsesProfile || (apiKey.trim() && baseURL.trim()))
    && (
