...
export const useStudioStore = create<StudioState>((set, get) => ({
  ...
  optimizePrompt: async (options: { useGuidance?: boolean } = {}) => {
    const s = get();
    if (s.isRunning || s.isOptimizingPrompt || s.isReversingPrompt) return;
  ...
  reversePromptFromImage: async () => {
    ...
    if (!reverseProfile.baseURL) {
      s.pushToast("先在上游配置里填入可用于反推提示词的 Responses API 地址", "warn", 5000);
      return;
    }
    ...
    if (current?.previewOnly) {
      ...
      if (materialized && useStudioStore.getState().currentImage?.id === current.id) {
    ...
    if (sourcePaths.length === 0 && sourceImages.length === 0) {
      s.pushToast("先生成或导入一张图片", "warn", 3600);
      return;
    }
