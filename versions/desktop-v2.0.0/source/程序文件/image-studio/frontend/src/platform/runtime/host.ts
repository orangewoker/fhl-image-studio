...
async function startRemoteJob(options: GenerateOptionsLike): Promise<JobStartedLike> {
  ...
  void (async () => {
    try {
      ...
      const suggestedName = `image-${options.mode || "generate"}.${options.outputFormat || "png"}`;
      const projectSaved = await saveProjectImage("output", result.imageB64, suggestedName);
      ...
      emitLocalEvent(`result:${jobId}`, {
        ...
        savedPath: projectSaved?.path ?? saved.path,
...
export function SaveImageAs(imageB64: string, suggestedName: string): Promise<string> {
  ...
  return saveProjectImage("output", imageB64, suggestedName, mimeType).then((projectSaved) => {
    ...
    return saveByDownload(new Blob([Uint8Array.from(atob(imageB64), (ch) => ch.charCodeAt(0))], { type: mimeType }), suggestedName);
  });
}
