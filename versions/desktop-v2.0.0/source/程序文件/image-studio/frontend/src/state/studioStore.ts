...
export const useStudioStore = create<StudioState>((set, get) => ({
  ...
  submit: async () => {
    ...
    if (s.mode === "edit") {
      ...
      if (isBrowserTaskProxyMode() && editSourcePaths.some((filePath) => isVirtualImagePath(filePath))) {
        set({
    ...
    if (isBrowserTaskProxyMode()) {
      try {
        const response = await submitBrowserJobGroup({
          workspaceId,
          mode: s.mode,
          prompt: augmentedPrompt,
          size: resolvedSize,
  ...
  bootstrap: async () => {
    ...
    if (isBrowserTaskProxyMode()) {
      const loadedGroups = await Promise.all(restoredWorkspaces.map(async (workspace) => {
        try {
          const response = await listBrowserJobGroups(workspace.id);
    ...
    const restoredRunningJobs = isBrowserTaskProxyMode()
    ...
    set({
      ...
      strokes: [],
      maskDataURL: null,
      runningJobMeta: isBrowserTaskProxyMode() ? buildRunningJobMetaFromBrowserGroups(jobGroupsByWorkspace) : {},
      jobGroupsByWorkspace,
      apiMode,
      requestPolicy,
      baseURL,
